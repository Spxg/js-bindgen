...
use crate::{
	...
	IMPORT_OUTPUT_RESULT, IMPORT_SUSPENDING, ImportShimKind, PointerWidth,
};
...
impl<'a> Import<'a> {
	fn decode_with(
		...
		let shim_kind = if flags & IMPORT_CLOSURE_FACTORY == 0 {
			ImportShimKind::Normal
		} else {
			ImportShimKind::ClosureFactory
		};
